...
logging.basicConfig(filename='removing_music.log', encoding='utf-8', format='%(asctime)s - %(levelname)s - %(message)s',
                    level=logging.INFO)
...
def process_file(original_video: Path):
    ...
    try:
        ...
        if no_music_sound.exists():
            ...
            print(f'"{original_video.name}": vocal already separated, skipping separating vocal')
        else:
            logging.info(f'"{original_video.name}": start separating vocal...')
            print(f'"{original_video.name}": start separating vocal...')
            ...
            print(f'"{original_video.name}": vocal seperated successfully')
        ...
        print(f'"{original_video.name}": deleting vocal sound...')
        ...
        print(f'"{original_video.name}": vocal sound deleted successfully')
        ...
        print('the process was stopped by user')
        raise error
...
def get_original_video() -> Path | None:
    ...
    print('Looking for file to process in folder "input"...')
